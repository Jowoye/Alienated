...
public class TileManager : MonoBehaviour
{
    ...
    /*float[] px (float lat, float lon, int zoom) {
		float d = (_settings.size / 2 * Mathf.Pow (_settings.zoom, 2));
    ...
    void Update()
    {
        ...
        target.position = Vector3.Lerp(target.position, new Vector3(0, 0.25f, 0), 1.0f * Time.deltaTime);
    }
    ...
    public class Settings
    {
        ...
        public int range = 1;
        [SerializeField]
        public string key = "pk.eyJ1Ijoic29sZXIiLCJhIjoiY2o4cnhyYWczMDJrZzMzcDBybXhsa3JndSJ9.x50x2ch5B9QXcdwsbmMZ6g";
        [SerializeField]
